...
def blacklist_add(user_id: int, reason: str, until_iso: Optional[str]):
    with get_conn() as conn:
        conn.execute(
            "INSERT OR REPLACE INTO blacklist(user_id, added_at, reason, until) VALUES(?,?,?,?)",
            (user_id, now_iso(), reason, until_iso)
        )

def blacklist_del(user_id: int):
    with get_conn() as conn:
        conn.execute("DELETE FROM blacklist WHERE user_id=?", (user_id,))

def blacklist_get(user_id: int) -> Optional[Dict[str, Any]]:
    with get_conn() as conn:
        row = conn.execute("SELECT * FROM blacklist WHERE user_id=?", (user_id,)).fetchone()
        if not row:
            return None
        return dict(row)

def is_blacklisted_active(user_id: int) -> bool:
    data = blacklist_get(user_id)
    if not data:
        return False
    ...
    try:
        ...
        return True
    return datetime.now(tz=UTC) < until_dt
...
def find_duplicate(category: str, description: str, attachment_unique_id: Optional[str]) -> Optional[int]:
    """
    find existing feedback within DUP_WINDOW_HOURS that matches.
    """
    ...
    with get_conn() as conn:
        ...
        for r in rows:
            if r["category"] != category:
                continue
            if (r["description"] or "").strip() != description.strip():
                continue
            # if both have attachments, also match unique id
            if attachment_unique_id and r["attachment_unique_id"]:
                if attachment_unique_id == r["attachment_unique_id"]:
                    return int(r["id"])
            else:
                # no attachment or one missing => treat as dup if text matches
                return int(r["id"])
    return None
...
def create_feedback(user_id: int, category: str, description: str,
    ...
    """
    returns (feedback_id, dedup_of_id)
    """
    ...
    with get_conn() as conn:
        ...
        if dedup_of:
            # increment dup_count on original
